<?php
/**
 * Medical Imaging Controller
 */
class ImagingController
{
    public function index(): void
    {
        $this->requirePermission('view_image');

        $search = $_GET['search'] ?? '';
        $imagingType = $_GET['imaging_type'] ?? '';
        $dateFrom = $_GET['date_from'] ?? '';
        $dateTo = $_GET['date_to'] ?? '';
        $page = max(1, (int) ($_GET['page_num'] ?? 1));

        $filters = [
            'search' => $search,
            'imaging_type' => $imagingType,
            'date_from' => $dateFrom,
            'date_to' => $dateTo,
            'limit' => RECORDS_PER_PAGE,
            'offset' => ($page - 1) * RECORDS_PER_PAGE,
        ];

        $images = MedicalImage::search($filters);
        $total = $this->countImages($filters);
        $totalPages = ceil($total / RECORDS_PER_PAGE);

        require APP_ROOT . '/views/imaging/index.php';
    }

    public function upload(): void
    {
        $this->requirePermission('upload_image');

        $errors = [];
        $old = [];
        $duplicateWarning = null;

        // Get patients for dropdown
        $patients = Patient::search(['status' => RECORD_ACTIVE, 'limit' => 1000]);

        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            CSRF::verify();

            $data = [
                'patient_id' => (int) ($_POST['patient_id'] ?? 0),
                'imaging_type' => $_POST['imaging_type'] ?? '',
                'study_date' => $_POST['study_date'] ?? '',
                'body_part' => trim($_POST['body_part'] ?? ''),
                'referring_clinician' => trim($_POST['referring_clinician'] ?? ''),
                'clinical_notes' => trim($_POST['clinical_notes'] ?? ''),
            ];

            $old = $data;

            // Validate fields
            $errors = $this->validateImageData($data);

            // Validate file
            if (empty($errors) && (!isset($_FILES['image_file']) || $_FILES['image_file']['error'] === UPLOAD_ERR_NO_FILE)) {
                $errors['image_file'] = 'Please select an image file to upload.';
            }

            if (empty($errors) && $_FILES['image_file']['error'] !== UPLOAD_ERR_OK) {
                $errors['image_file'] = 'File upload failed. Error code: ' . $_FILES['image_file']['error'];
            }

            if (empty($errors)) {
                $file = $_FILES['image_file'];

                // Validate file
                $fileErrors = $this->validateFile($file);
                if (!empty($fileErrors)) {
                    $errors = array_merge($errors, $fileErrors);
                }
            }

            if (empty($errors)) {
                $file = $_FILES['image_file'];

                // Calculate SHA-256 hash
                $hash = hash_file('sha256', $file['tmp_name']);

                // Check for duplicate
                $existing = MedicalImage::findByHash($hash);

                if ($existing) {
                    $duplicateWarning = $existing;

                    AuditLog::create([
                        'user_id' => Auth::id(),
                        'action' => AUDIT_IMAGE_DUPLICATE,
                        'entity_type' => 'medical_image',
                        'entity_id' => $existing['image_id'],
                        'description' => "Duplicate image upload attempt. Existing image ID: {$existing['image_id']}",
                    ]);
                } else {
                    // Save file with secure random name
                    $extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
                    $secureName = bin2hex(random_bytes(16)) . '.' . $extension;
                    $filePath = UPLOAD_DIR . $secureName;

                    if (!is_dir(UPLOAD_DIR)) {
                        mkdir(UPLOAD_DIR, 0755, true);
                    }

                    if (move_uploaded_file($file['tmp_name'], $filePath)) {
                        if (!file_exists($filePath)) {
                            error_log("Upload failure: file does not exist at {$filePath}");
                            $errors['image_file'] = 'Storage integrity error: file not saved.';
                            require APP_ROOT . '/views/imaging/upload.php';
                            return;
                        }

                        $actualSize = filesize($filePath);
                        if ($actualSize === 0 || $actualSize === false) {
                            error_log("Upload failure: file is empty at {$filePath}");
                            @unlink($filePath);
                            $errors['image_file'] = 'Storage integrity error: file is empty.';
                            require APP_ROOT . '/views/imaging/upload.php';
                            return;
                        }

                        $actualHash = hash_file('sha256', $filePath);
                        if ($actualHash !== $hash) {
                            error_log("Upload corruption: hash mismatch at {$filePath}");
                            @unlink($filePath);
                            $errors['image_file'] = 'Storage integrity error: hash mismatch.';
                            require APP_ROOT . '/views/imaging/upload.php';
                            return;
                        }

                        $imageId = MedicalImage::create([
                            'patient_id' => $data['patient_id'],
                            'imaging_type' => $data['imaging_type'],
                            'study_date' => $data['study_date'],
                            'body_part' => $data['body_part'],
                            'referring_clinician' => $data['referring_clinician'],
                            'clinical_notes' => $data['clinical_notes'],
                            'file_name' => $secureName,
                            'original_file_name' => $file['name'],
                            'file_path' => 'uploads/medical_images/' . $secureName,
                            'file_mime_type' => mime_content_type($filePath),
                            'file_size' => $file['size'],
                            'file_hash' => $hash,
                            'uploaded_by' => Auth::id(),
                        ]);

                        AuditLog::create([
                            'user_id' => Auth::id(),
                            'action' => AUDIT_IMAGE_UPLOAD,
                            'entity_type' => 'medical_image',
                            'entity_id' => $imageId,
                            'description' => "Uploaded {$data['imaging_type']} image for patient ID {$data['patient_id']}",
                        ]);

                        Session::flash('success', 'Medical image uploaded successfully.');
                        header('Location: ' . BASE_URL . '/index.php?page=imaging&action=view&id=' . $imageId);
                        exit;
                    } else {
                        $errors['image_file'] = 'Failed to save uploaded file. Check directory permissions.';
                    }
                }
            }
        }

        require APP_ROOT . '/views/imaging/upload.php';
    }

    public function view(): void
    {
        $this->requirePermission('view_image');

        $id = (int) ($_GET['id'] ?? 0);

        $stmt = Database::getInstance()->prepare("
            SELECT mi.*, 
                   CONCAT(p.first_name, ' ', p.last_name) as patient_name,
                   p.hospital_number,
                   p.patient_id,
                   u.full_name as uploaded_by_name
            FROM medical_images mi
            INNER JOIN patients p ON mi.patient_id = p.patient_id
            LEFT JOIN users u ON mi.uploaded_by = u.user_id
            WHERE mi.image_id = ?
            LIMIT 1
        ");
        $stmt->execute([$id]);
        $image = $stmt->fetch();

        if (!$image) {
            http_response_code(404);
            require APP_ROOT . '/views/errors/404.php';
            return;
        }

        AuditLog::create([
            'user_id' => Auth::id(),
            'action' => AUDIT_IMAGE_VIEW,
            'entity_type' => 'medical_image',
            'entity_id' => $id,
            'description' => "Viewed image record ID: {$id}",
        ]);

        require APP_ROOT . '/views/imaging/view.php';
    }

    public function serve(): void
    {
        $this->requirePermission('view_image');

        $id = (int) ($_GET['id'] ?? 0);
        $image = MedicalImage::find($id);

        if (!$image) {
            http_response_code(404);
            header('Content-Type: text/plain');
            echo 'Image record not found.';
            exit;
        }

        $integrity = StorageIntegrity::checkImage($image);

        if ($integrity['status'] !== StorageIntegrity::STATUS_OK
            && $integrity['status'] !== StorageIntegrity::STATUS_SIZE_MISMATCH) {

            AuditLog::create([
                'user_id'     => Auth::id(),
                'action'      => 'IMAGE_INTEGRITY_FAILURE',
                'entity_type' => 'medical_image',
                'entity_id'   => $id,
                'description' => "Integrity check failed for image #{$id}: {$integrity['message']}",
            ]);

            http_response_code(404);
            header('Content-Type: text/plain');
            echo "Image file is unavailable.
";
            echo "Status: {$integrity['status']}
";
            echo "Details: {$integrity['message']}
";
            exit;
        }

        $filePath = APP_ROOT . '/' . ltrim($image['file_path'], '/');

        header('Content-Type: ' . $image['file_mime_type']);
        header('Content-Length: ' . filesize($filePath));
        header('Content-Disposition: inline; filename="' . basename($image['file_name']) . '"');
        header('X-Content-Type-Options: nosniff');
        header('Cache-Control: private, max-age=3600');

        readfile($filePath);
        exit;
    }

    public function download(): void
    {
        $this->requirePermission('view_image');

        $id = (int) ($_GET['id'] ?? 0);
        $image = MedicalImage::find($id);

        if (!$image) {
            http_response_code(404);
            exit;
        }

        AuditLog::create([
            'user_id' => Auth::id(),
            'action' => AUDIT_IMAGE_DOWNLOAD,
            'entity_type' => 'medical_image',
            'entity_id' => $id,
            'description' => "Downloaded image: {$image['original_file_name']}",
        ]);

        $filePath = APP_ROOT . '/' . $image['file_path'];

        if (!file_exists($filePath)) {
            http_response_code(404);
            exit;
        }

        header('Content-Type: ' . $image['file_mime_type']);
        header('Content-Length: ' . filesize($filePath));
        header('Content-Disposition: attachment; filename="' . $image['original_file_name'] . '"');
        readfile($filePath);
        exit;
    }

    public function archive(): void
    {
        $this->requirePermission('archive_image');

        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            http_response_code(405);
            exit;
        }

        CSRF::verify();

        $id = (int) ($_POST['id'] ?? 0);
        $image = MedicalImage::find($id);

        if (!$image) {
            http_response_code(404);
            require APP_ROOT . '/views/errors/404.php';
            return;
        }

        MedicalImage::update($id, ['record_status' => RECORD_ARCHIVED]);

        AuditLog::create([
            'user_id' => Auth::id(),
            'action' => AUDIT_IMAGE_ARCHIVE,
            'entity_type' => 'medical_image',
            'entity_id' => $id,
            'description' => "Archived image record ID: {$id}",
        ]);

        Session::flash('success', 'Image record archived successfully.');
        header('Location: ' . BASE_URL . '/index.php?page=imaging');
        exit;
    }

    private function validateImageData(array $data): array
    {
        $errors = [];

        if (empty($data['patient_id'])) {
            $errors['patient_id'] = 'Please select a patient.';
        } elseif (!Patient::find($data['patient_id'])) {
            $errors['patient_id'] = 'Selected patient does not exist.';
        }

        if (empty($data['imaging_type']) || !in_array($data['imaging_type'], IMAGING_TYPES, true)) {
            $errors['imaging_type'] = 'Please select a valid imaging type.';
        }

        if (empty($data['study_date'])) {
            $errors['study_date'] = 'Study date is required.';
        } elseif (strtotime($data['study_date']) > time()) {
            $errors['study_date'] = 'Study date cannot be in the future.';
        }

        if (empty($data['body_part'])) {
            $errors['body_part'] = 'Body part is required.';
        }

        return $errors;
    }

    private function validateFile(array $file): array
    {
        $errors = [];

        if ($file['size'] > MAX_FILE_SIZE) {
            $errors['image_file'] = 'File size exceeds maximum allowed (' . (MAX_FILE_SIZE / 1024 / 1024) . ' MB).';
            return $errors;
        }

        $extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        if (!in_array($extension, ALLOWED_EXTENSIONS, true)) {
            $errors['image_file'] = 'Invalid file type. Allowed: ' . implode(', ', ALLOWED_EXTENSIONS);
            return $errors;
        }

        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $mimeType = $finfo->file($file['tmp_name']);
        if (!in_array($mimeType, ALLOWED_MIME_TYPES, true)) {
            $errors['image_file'] = 'Invalid file content type. Only JPEG and PNG images are allowed.';
            return $errors;
        }

        // Verify it's a real image
        if (!@getimagesize($file['tmp_name'])) {
            $errors['image_file'] = 'The uploaded file is not a valid image.';
        }

        return $errors;
    }

    private function countImages(array $filters): int
    {
        $sql = "SELECT COUNT(*) FROM medical_images mi 
                INNER JOIN patients p ON mi.patient_id = p.patient_id WHERE 1=1";
        $params = [];

        if (!empty($filters['search'])) {
            $sql .= " AND (p.hospital_number LIKE ? OR p.first_name LIKE ? OR p.last_name LIKE ? 
                          OR mi.body_part LIKE ?)";
            $term = '%' . $filters['search'] . '%';
            $params = array_merge($params, [$term, $term, $term, $term]);
        }

        if (!empty($filters['imaging_type'])) {
            $sql .= " AND mi.imaging_type = ?";
            $params[] = $filters['imaging_type'];
        }

        $stmt = Database::getInstance()->prepare($sql);
        $stmt->execute($params);
        return (int) $stmt->fetchColumn();
    }

    private function requirePermission(string $permission): void
    {
        if (!Auth::check()) {
            header('Location: ' . BASE_URL . '/login.php');
            exit;
        }
        if (!Auth::hasPermission($permission)) {
            http_response_code(403);
            require APP_ROOT . '/views/errors/403.php';
            exit;
        }
    }

    /**
     * Admin: Directly archive an image (requires reason)
     */
    public function delete(): void
    {
        $this->requirePermission('delete_image');

        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); exit; }
        CSRF::verify();

        $id = (int) ($_POST['id'] ?? 0);
        $reason = trim($_POST['reason'] ?? '');

        if (empty($reason)) {
            Session::flash('error', 'Please provide a reason for deleting this image.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=view&id=' . $id);
            exit;
        }

        if (strlen($reason) < 10) {
            Session::flash('error', 'Please provide a more detailed reason (at least 10 characters).');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=view&id=' . $id);
            exit;
        }

        $image = MedicalImage::find($id);
        if (!$image) {
            Session::flash('error', 'Image not found.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging');
            exit;
        }

        MedicalImage::update($id, ['record_status' => 'archived']);

        AuditLog::create([
            'user_id' => Auth::id(),
            'action' => 'IMAGE_DELETE',
            'entity_type' => 'medical_image',
            'entity_id' => $id,
            'description' => "Admin deleted image #{$id} - Reason: {$reason}",
        ]);

        Session::flash('success', 'Image deleted successfully.');
        header('Location: ' . BASE_URL . '/index.php?page=imaging');
        exit;
    }

    /**
     * Technician: Request deletion for admin approval
     */
    public function requestDelete(): void
    {
        $this->requirePermission('request_delete_image');

        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); exit; }
        CSRF::verify();

        $id = (int) ($_POST['id'] ?? 0);
        $reason = trim($_POST['reason'] ?? '');

        if (empty($reason) || strlen($reason) < 10) {
            Session::flash('error', 'Please provide a reason of at least 10 characters.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=view&id=' . $id);
            exit;
        }

        $image = MedicalImage::find($id);
        if (!$image) {
            Session::flash('error', 'Image not found.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging');
            exit;
        }

        $db = Database::getInstance();

        $stmt = $db->prepare("SELECT COUNT(*) FROM deletion_requests WHERE image_id = ? AND request_status = 'pending'");
        $stmt->execute([$id]);
        if ($stmt->fetchColumn() > 0) {
            Session::flash('warning', 'A deletion request for this image is already pending.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=view&id=' . $id);
            exit;
        }

        $db->prepare("INSERT INTO deletion_requests (image_id, requested_by, reason) VALUES (?, ?, ?)")
           ->execute([$id, Auth::id(), $reason]);
        $requestId = (int) $db->lastInsertId();

        AuditLog::create([
            'user_id' => Auth::id(),
            'action' => 'IMAGE_DELETE_REQUEST',
            'entity_type' => 'deletion_request',
            'entity_id' => $requestId,
            'description' => "Deletion requested for image #{$id} - Reason: {$reason}",
        ]);

        Session::flash('success', 'Deletion request submitted. Awaiting administrator approval.');
        header('Location: ' . BASE_URL . '/index.php?page=imaging');
        exit;
    }

    /**
     * Admin: View pending deletion requests
     */
    public function deletionRequests(): void
    {
        $this->requirePermission('approve_deletion');
        $db = Database::getInstance();

        $requests = $db->query("
            SELECT dr.*,
                   mi.imaging_type, mi.study_date, mi.body_part, mi.original_file_name,
                   mi.image_id,
                   p.first_name AS patient_first, p.last_name AS patient_last, p.hospital_number,
                   u.full_name AS requester_name, u.username AS requester_username
            FROM deletion_requests dr
            INNER JOIN medical_images mi ON dr.image_id = mi.image_id
            INNER JOIN patients p ON mi.patient_id = p.patient_id
            INNER JOIN users u ON dr.requested_by = u.user_id
            WHERE dr.request_status = 'pending'
            ORDER BY dr.created_at DESC
        ")->fetchAll();

        $history = $db->query("
            SELECT dr.*,
                   mi.imaging_type, mi.body_part, mi.image_id,
                   u.full_name AS requester_name,
                   r.full_name AS reviewer_name
            FROM deletion_requests dr
            INNER JOIN medical_images mi ON dr.image_id = mi.image_id
            INNER JOIN users u ON dr.requested_by = u.user_id
            LEFT JOIN users r ON dr.reviewed_by = r.user_id
            WHERE dr.request_status IN ('approved', 'rejected')
            ORDER BY dr.reviewed_at DESC
            LIMIT 20
        ")->fetchAll();

        $pageTitle = 'Deletion Requests';
        require APP_ROOT . '/views/imaging/deletion_requests.php';
    }

    /**
     * Admin: Approve or reject a deletion request
     */
    public function reviewDeletion(): void
    {
        $this->requirePermission('approve_deletion');

        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); exit; }
        CSRF::verify();

        $requestId = (int) ($_POST['request_id'] ?? 0);
        $decision  = $_POST['decision'] ?? '';
        $notes     = trim($_POST['admin_notes'] ?? '');

        if (!in_array($decision, ['approve', 'reject'], true)) {
            Session::flash('error', 'Invalid decision.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=deletionRequests');
            exit;
        }

        $db = Database::getInstance();
        $stmt = $db->prepare("SELECT * FROM deletion_requests WHERE request_id = ? AND request_status = 'pending'");
        $stmt->execute([$requestId]);
        $request = $stmt->fetch();

        if (!$request) {
            Session::flash('error', 'Request not found or already processed.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=deletionRequests');
            exit;
        }

        $db->beginTransaction();
        try {
            $newStatus = ($decision === 'approve') ? 'approved' : 'rejected';

            $db->prepare("UPDATE deletion_requests
                          SET request_status = ?, reviewed_by = ?, reviewed_at = NOW(), admin_notes = ?
                          WHERE request_id = ?")
               ->execute([$newStatus, Auth::id(), $notes ?: null, $requestId]);

            if ($decision === 'approve') {
                $db->prepare("UPDATE medical_images SET record_status = 'archived' WHERE image_id = ?")
                   ->execute([$request['image_id']]);
            }

            AuditLog::create([
                'user_id' => Auth::id(),
                'action' => ($decision === 'approve') ? 'IMAGE_DELETE_APPROVED' : 'IMAGE_DELETE_REJECTED',
                'entity_type' => 'deletion_request',
                'entity_id' => $requestId,
                'description' => "Deletion request #{$requestId} {$newStatus}" . ($notes ? " - Notes: {$notes}" : ''),
            ]);

            $db->commit();
        } catch (Exception $e) {
            $db->rollBack();
            error_log('Deletion review failed: ' . $e->getMessage());
            Session::flash('error', 'Failed to process the request.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=deletionRequests');
            exit;
        }

        $msg = ($decision === 'approve')
            ? 'Request approved. Image archived.'
            : 'Request rejected. Image remains active.';
        Session::flash('success', $msg);
        header('Location: ' . BASE_URL . '/index.php?page=imaging&action=deletionRequests');
        exit;
    }

    /**
     * Admin: View recently deleted (archived) images with full context
     */
    public function recentlyDeleted(): void
    {
        $this->requirePermission('delete_image');
        $db = Database::getInstance();

        $deleted = $db->query("
            SELECT mi.*,
                   CONCAT(p.first_name, ' ', p.last_name) AS patient_name,
                   p.hospital_number,
                   u_direct.full_name AS deleted_by_name,
                   (SELECT dr.reason 
                    FROM deletion_requests dr 
                    WHERE dr.image_id = mi.image_id 
                      AND dr.request_status = 'approved'
                    ORDER BY dr.reviewed_at DESC 
                    LIMIT 1) AS deletion_reason,
                   (SELECT u.full_name 
                    FROM deletion_requests dr 
                    INNER JOIN users u ON dr.requested_by = u.user_id
                    WHERE dr.image_id = mi.image_id 
                      AND dr.request_status = 'approved'
                    ORDER BY dr.reviewed_at DESC 
                    LIMIT 1) AS requested_by_name,
                   (SELECT dr.reviewed_at 
                    FROM deletion_requests dr 
                    WHERE dr.image_id = mi.image_id 
                      AND dr.request_status = 'approved'
                    ORDER BY dr.reviewed_at DESC 
                    LIMIT 1) AS approved_at
            FROM medical_images mi
            INNER JOIN patients p ON mi.patient_id = p.patient_id
            LEFT JOIN users u_direct ON mi.uploaded_by = u_direct.user_id
            WHERE mi.record_status = 'archived'
            ORDER BY mi.updated_at DESC
            LIMIT 200
        ")->fetchAll();

        $stats = $db->query("
            SELECT 
                COUNT(*) AS total,
                SUM(CASE WHEN DATE(updated_at) = CURDATE() THEN 1 ELSE 0 END) AS today,
                SUM(CASE WHEN updated_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN 1 ELSE 0 END) AS this_week
            FROM medical_images
            WHERE record_status = 'archived'
        ")->fetch();

        $pageTitle = 'Recently Deleted Images';
        require APP_ROOT . '/views/imaging/recently_deleted.php';
    }

    /**
     * Admin: Restore an archived image
     */
    public function restore(): void
    {
        $this->requirePermission('delete_image');

        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); exit; }
        CSRF::verify();

        $id = (int) ($_POST['id'] ?? 0);
        $image = MedicalImage::find($id);

        if (!$image) {
            Session::flash('error', 'Image not found.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=recentlyDeleted');
            exit;
        }

        if ($image['record_status'] === 'active') {
            Session::flash('warning', 'Image is already active.');
            header('Location: ' . BASE_URL . '/index.php?page=imaging&action=recentlyDeleted');
            exit;
        }

        MedicalImage::update($id, ['record_status' => 'active']);

        AuditLog::create([
            'user_id' => Auth::id(),
            'action' => 'IMAGE_RESTORE',
            'entity_type' => 'medical_image',
            'entity_id' => $id,
            'description' => "Restored archived image #{$id}",
        ]);

        Session::flash('success', 'Image restored successfully. It is now active again.');
        header('Location: ' . BASE_URL . '/index.php?page=imaging&action=recentlyDeleted');
        exit;
    }

    /**
     * Admin/Staff: View all archived images with search & filter
     */
    public function archived(): void
    {
        $this->requirePermission('view_image');
        $db = Database::getInstance();

        $search = $_GET['search'] ?? '';
        $imagingType = $_GET['imaging_type'] ?? '';
        $dateFrom = $_GET['date_from'] ?? '';
        $dateTo = $_GET['date_to'] ?? '';
        $page = max(1, (int) ($_GET['page_num'] ?? 1));
        $limit = RECORDS_PER_PAGE;
        $offset = ($page - 1) * $limit;

        $sql = "SELECT mi.*,
                       CONCAT(p.first_name, ' ', p.last_name) as patient_name,
                       p.hospital_number,
                       u.full_name as uploaded_by_name,
                       (SELECT dr.reason FROM deletion_requests dr 
                        WHERE dr.image_id = mi.image_id AND dr.request_status = 'approved'
                        ORDER BY dr.reviewed_at DESC LIMIT 1) AS deletion_reason,
                       (SELECT u2.full_name FROM deletion_requests dr
                        INNER JOIN users u2 ON dr.reviewed_by = u2.user_id
                        WHERE dr.image_id = mi.image_id AND dr.request_status = 'approved'
                        ORDER BY dr.reviewed_at DESC LIMIT 1) AS deleted_by_name
                FROM medical_images mi
                INNER JOIN patients p ON mi.patient_id = p.patient_id
                LEFT JOIN users u ON mi.uploaded_by = u.user_id
                WHERE mi.record_status = 'archived'";
        $params = [];

        if ($search) {
            $sql .= " AND (p.hospital_number LIKE ? OR p.first_name LIKE ? OR p.last_name LIKE ? 
                          OR mi.body_part LIKE ?)";
            $term = "%{$search}%";
            $params = array_merge($params, [$term, $term, $term, $term]);
        }
        if ($imagingType) {
            $sql .= " AND mi.imaging_type = ?";
            $params[] = $imagingType;
        }
        if ($dateFrom) {
            $sql .= " AND mi.study_date >= ?";
            $params[] = $dateFrom;
        }
        if ($dateTo) {
            $sql .= " AND mi.study_date <= ?";
            $params[] = $dateTo;
        }

        // Count for pagination
        $countSql = preg_replace('/SELECT mi\.\*.*?FROM/s', 'SELECT COUNT(*) FROM', $sql, 1);
        $countSql = str_replace('ORDER BY mi.study_date DESC', '', $countSql);
        $countSql = str_replace('ORDER BY mi.study_date DESC, mi.uploaded_at DESC', '', $countSql);
        // Simple count query - avoid the ORDER BY issue
        $countSql = "SELECT COUNT(*) FROM medical_images mi
                     INNER JOIN patients p ON mi.patient_id = p.patient_id
                     WHERE mi.record_status = 'archived'";
        $countParams = [];
        if ($search) {
            $countSql .= " AND (p.hospital_number LIKE ? OR p.first_name LIKE ? OR p.last_name LIKE ? OR mi.body_part LIKE ?)";
            $countParams = array_merge($countParams, [$term, $term, $term, $term]);
        }
        if ($imagingType) {
            $countSql .= " AND mi.imaging_type = ?";
            $countParams[] = $imagingType;
        }
        if ($dateFrom) {
            $countSql .= " AND mi.study_date >= ?";
            $countParams[] = $dateFrom;
        }
        if ($dateTo) {
            $countSql .= " AND mi.study_date <= ?";
            $countParams[] = $dateTo;
        }

        $stmt = $db->prepare($countSql);
        $stmt->execute($countParams);
        $total = (int) $stmt->fetchColumn();
        $totalPages = ceil($total / $limit);

        // Add ORDER BY and pagination
        $sql .= " ORDER BY mi.updated_at DESC LIMIT " . (int) $limit . " OFFSET " . (int) $offset;

        $stmt = $db->prepare($sql);
        $stmt->execute($params);
        $images = $stmt->fetchAll();

        // Get stats
        $stats = $db->query("
            SELECT 
                COUNT(*) AS total,
                SUM(CASE WHEN imaging_type = 'X-ray' THEN 1 ELSE 0 END) AS xray,
                SUM(CASE WHEN imaging_type = 'CT' THEN 1 ELSE 0 END) AS ct,
                SUM(CASE WHEN imaging_type = 'MRI' THEN 1 ELSE 0 END) AS mri,
                SUM(CASE WHEN imaging_type = 'Ultrasound' THEN 1 ELSE 0 END) AS ultrasound
            FROM medical_images
            WHERE record_status = 'archived'
        ")->fetch();

        $pageTitle = 'Archived Images';
        require APP_ROOT . '/views/imaging/archived.php';
    }
}
